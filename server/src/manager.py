...
class Manager:
    def __init__(self, root=None, database=None):
        ...
        self.manager.spacer = tk.Label(
            self.manager, height=4, width=5, anchor=tk.E)
        ...
        self.cols_book = ("ID", "Title", "Author",
                          "Published year", "Type", "Link")
        self.cols_width_book = (80, 250, 160, 100, 100, 200)
        self.cols_account = ("Username", "Password")
        self.manager.result = ttk.Treeview(
            self.manager, columns=self.cols_book, height=25, show="headings")
        self.manager.scroll_bar = ttk.Scrollbar(
            self.manager, command=self.manager.result.yview)
        self.manager.result.configure(
            yscrollcommand=self.manager.scroll_bar.set)
        ...
        # Draw widgets
        self.manager.btn_insert.grid(
            column=1, row=0, sticky="w", padx=10, pady=20, columnspan=2)
        self.manager.btn_update.grid(
            column=2, row=0, sticky="n", padx=10, pady=20, columnspan=2)
        self.manager.btn_view.grid(
            column=4, row=0, sticky="w", padx=10, pady=20, columnspan=2)
        self.manager.result.grid(
            column=0, row=2, sticky='nsew', pady=30, columnspan=len(self.cols_book))
        self.manager.scroll_bar.grid(column=len(
            self.cols_book), row=2, columnspan=1)
    ...
    def create_input_box(self):
        ...
        self.input_box.lbl_notice = tk.Label(
            self.input_box, text="", font="Consolas 14")
        self.input_box.lbl = [None] * len(self.cols_book)
        self.input_box.ety = [None] * len(self.cols_book)
        for idx in range(len(self.cols_book)):
            self.input_box.lbl[idx] = tk.Label(
                self.input_box, text=self.cols_book[idx], font="Consolas 14 bold")
            self.input_box.ety[idx] = tk.Entry(
                self.input_box, bd=2, width=60 if idx != 5 else 52, font="Consolas 14")
            self.input_box.lbl[idx].grid(
                row=idx, column=0, padx=10, pady=10, sticky="w")
            self.input_box.ety[idx].grid(row=idx, column=1, padx=5, sticky="w")
        self.input_box.btn_browser = tk.Button(
            self.input_box, text="Browser...", command=self.fileDialog)
        self.input_box.btn_insert = tk.Button(
            self.input_box, text="INSERT", font="Consolas 20 bold", command=self.on_insert)
        self.input_box.btn_clear = tk.Button(
            self.input_box, text="CLEAR", font="Consolas 20 bold", command=self.on_clear)

        self.input_box.lbl_notice.grid(
            row=len(self.cols_book), column=0, columnspan=3, sticky="n")

        self.input_box.btn_browser.grid(row=5, column=1, padx=5, sticky="e")
        self.input_box.btn_insert.grid(
            row=len(self.cols_book) + 1, column=1, pady=20, sticky="w", columnspan=1)
        self.input_box.btn_clear.grid(row=len(
            self.cols_book) + 1, column=1, pady=20, padx=150, sticky="e", columnspan=1)

    def fileDialog(self, event=None):
        filename = filedialog.askopenfilename(
            parent=self.input_box, title="Select a book file")
        if not filename:
            return
        cwd = os.getcwd()
    ...
    def update_book(self):
        ...
        self.input_box.btn_insert.configure(
            text="UPDATE BOOK", command=self.on_update)
        self.input_box.btn_clear.configure(
            text="DELETE BOOK", command=self.on_delete)
        self.input_box.ety[0].configure(width=52)
        self.input_box.btn_check = tk.Button(
            self.input_box, text="GET BOOK", command=self.on_get_book)
    ...
    def get_book_data(self):
        ...
        for idx in range(len(self.cols_book)):
            ...
            self.input_box.lbl_notice.configure(
                fg="red")  # set red color error
            if not entry_text:
            ...
            if (col == "Published year"):
                if (len(entry_text) != 4) or (not entry_text.isdigit()):
                    self.input_box.lbl_notice.configure(
                        text="Please enter correct year's format\n(ex: 2020, 1900, ...)")
                    return
            if (col == "ID"):
                if (len(entry_text) > 5):
                    self.input_box.lbl_notice.configure(
                        text="Please enter Book ID no more than 5 character\n(ex: CS001, 12345,...)")
                    return
                if not entry_text.isalnum():
                    self.input_box.lbl_notice.configure(
                        text="Please enter Book ID contains only [A-z][0-9] character")
                    return
    ...
    def on_delete(self, event=None):
        ...
        self.reset_update_book()
        self.input_box.lbl_notice.configure(
            text="Delete book successfully", fg="green")
    ...
    def on_update(self, event=None):
        ...
        self.input_box.lbl_notice.configure(
            text="Update book successfully", fg="green")
