...
class Login(tk.Frame):
    ...
    def create_widgets(self):
        self.spacer = tk.Label(self, height=5, width=26, anchor=tk.E)
        self.spacer.grid(row=0, column=0)

        self.lbl_username = tk.Label(
            self, height=1, text="Username:", width=20, justify="left", font=style.label_font, anchor=tk.W)
        ...
        self.lbl_password = tk.Label(self, height=1, text="Password:",
                                     width=20, justify="left", font=style.label_font, anchor=tk.W)
        self.lbl_password.grid(row=3, column=1, sticky=tk.E+tk.W,
                               padx=10, pady=0, columnspan=2)
        self.txt_password = tk.Entry(
            self, show="*", width=36, bg="#FFFFFF", font=style.entry_font)
        self.txt_password.grid(row=4, column=1, sticky=tk.E,
                               padx=10, pady=10, columnspan=2)

        self.btn_login = tk.Button(
            self, text="Log in", width=10, font=style.btn_font,
            activebackground=style.btn_style['actbg'], bg=style.btn_style['bg'], fg=style.btn_style['fg'])
        self.btn_login.grid(row=5, column=1, sticky=tk.E, ipadx=10,
                            padx=20, pady=10, columnspan=1)

        self.btn_newacc = tk.Button(
            self, text="New account", width=10, font=style.btn_font,
            activebackground=style.btn_style['actbg'], bg=style.btn_style['bg'], fg=style.btn_style['fg'])
        self.btn_newacc.grid(row=5, column=2, sticky=tk.W, ipadx=10,
                             padx=20, pady=10, columnspan=1)
